...
3. **Load guidance:** Apply any relevant repo docs/rules/command guidance for the areas being changed.
...
6. **Implement iteratively:** Make small changes, update/add tests per acceptance criteria, and keep commits atomic.
  - If committing, use `/issue-commit` and follow these commit rules:
    - Types: `feat`, `fix`, `docs`, `style`, `refactor`, `test`, `chore`, `perf`, `ci`, `build`, `revert`.
    - Format: `type(scope): subject` (scope optional).
    - Header <= 100 chars; subject imperative; no trailing period; no emojis.
    - Include `ref <issue-key>` in the body only when applicable.
7. **Verify:** Run relevant checks during implementation. Before handoff, run the quality gate or note why it is deferred.
